...
All arguments and descriptions can be viewed by running `sparrow --help`. Below we include the arguments we expect to be most relevant to users. 
 ...
 - `--reward-weight` ^L^: weighting factor for reward objective 
 - `--start-cost-weight` ^L^: weighting factor for starting material cost objective
 - `--reaction-weight` ^L^: weighting factor for reaction objective
 - `--max-rxns` ^E^: maximum number of reaction steps to select
 - `--starting-material-budget` ^E^: maximum starting material cost 
 ...
 - `--diversity-weight` (default: `0`) ^L^: A weighting factor that encourages selection of many similarity-based clusters. 
 ...
 - `--solver {pulp, gurobi}` (default: `pulp`) ^L^: the solver to use. The expected reward formulation requires and defaults to Gurobi. 
^L^ only used in the linear formulation
^E^ only used in the expected reward formulation
