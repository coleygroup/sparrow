...
All arguments and descriptions can be viewed by running `sparrow --help`. Below we include the arguments we expect to be most relevant to users. 
 ...
 - `--formulation {linear, expected_reward}` (default: `linear`): which formulation to use. `linear` is the formulation optimized in our published paper, while `expected_reward` directly optimizes expected reward (see published paper, Figure 2)
...
When running Bayesian optimization on the weighting factors, a series of subfolders for each examined set of weights will be created in the output folder. Each subfolder will have a `summary.json` file, as specified above. The overall output folder will also contain a `bayesian_summary.json` file, which will contain the parameters and full output of the Bayesian run, the optimal solution and weighting factors, as well as the contents of the `summary.json` file of the optimal solution. When running Bayseian optimization, SCALE ALL INPUT WEIGHTS to [0,1].
